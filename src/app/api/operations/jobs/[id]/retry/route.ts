...
import { retryJob } from "@/services/operations";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await retryJob(tenantId, (await params).id);
  if (!result.ok) {
    return apiError(
      result.reason === "not_found" ? "job not found" : "job is not retryable",
