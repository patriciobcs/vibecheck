...
The landing page's primary **Add your project** CTA leads to `/products/new`, preserving that destination through sign-in. Account entry always visits the sign-in route; it no longer silently chooses a shared demo account. Signed-in users skip email and see their identity on the project form. The two visible steps are **Sign in** and **Add your repository**. Project name and GitHub repository URL are required; the live app URL and other context are optional at creation (VC-01). Email belongs to account access, not project data.

Shared demo access is an explicit alternative on sign-in and labels the project form as a shared workspace. The same form is used in all modes. `EMAIL_MODE=test_inbox` exposes the local inbox only in development and says no email was sent. Production email mode uses Resend (`RESEND_API_KEY` plus verified `EMAIL_FROM`); unavailable delivery is shown as unavailable and rejected on the server. Explicit demo sign-in never sends a real email, even when Resend is configured. Return paths accept only internal destinations.
...
A user without memberships receives a private owner workspace only when submitting a valid first project. Creation locks the user row and commits the tenant, owner membership and product together; concurrent submissions reuse the membership. Existing roles are preserved, viewers cannot add projects, and owners with several writable workspaces must choose one explicitly. Saving a project does not activate monitoring, issue publication or repair. The repository card distinguishes a saved link from authorized GitHub access and keeps writes off. General product editing and the GitHub installation wizard remain unimplemented.

### Product overview

Show the live product URL when configured, repository binding and credential state, experiments (studies) by stage, findings needing attention, open issues and draft PRs, the latest experiment summary and a Signals (research candidates) panel. A saved repository alone is not a healthy GitHub connection. Without a live URL, show an authorized setup form and disable discovery; direct API/worker calls also enforce the prerequisite. Empty states give one clear next action: add live app, run discovery or publish an experiment. Lists and the live-monitoring waiting screen tolerate a missing app URL. Avoid fabricated ROI, participation counts or generic honesty/UX scores; numbers come from persisted rows with denominators.
