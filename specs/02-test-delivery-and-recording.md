...
- **Auth.** Better Auth magic links for owners and signed-in participants; one user table with tenant memberships for owners and a participant profile per user. `EMAIL_MODE=test_inbox` writes links to `notification_outbox` and `/dev/inbox` only in development; normal production sign-in rejects this mode. `EMAIL_MODE=resend` uses the Resend email API with `RESEND_API_KEY` and verified `EMAIL_FROM`, records `sent` only after provider acceptance, and surfaces send failures. Live delivery is not yet verified with deployment credentials. Explicit shared-demo entry scopes its generated link to the test inbox and never emails the seeded identities, including when real delivery is enabled. Existing sessions skip sign-in and internal return paths are preserved.
- **Research setup.** Repository-first onboarding may save a product without a live app URL. Such projects cannot publish studies; assignment views and handoff URLs also reject a missing target rather than opening GitHub or a placeholder as the participant's task.
